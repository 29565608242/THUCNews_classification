...
| 模型         | Accuracy   | Macro F1   | Macro Precision | Macro Recall |
|--------------|------------|------------|-----------------|--------------|
| **BERT**     | **0.9761** | **0.9738** | —               | —            |
| TF-IDF+SVM   | 0.9557     | 0.9503     | —               | —            |
| BiLSTM       | 0.9507     | 0.9381     | —               | —            |
...
- **TF-IDF+SVM 性价比极高**：即使在全量数据下 F1 也达到 0.9503，训练仅需数秒
- **数据量增益递减**：从 20k→100k 提升显著，之后边际收益递减
- **BiLSTM 介于两者之间**，但调参得当后在全量数据下接近 SVM
- **小数据量下 BERT 优势更突出**（20k 时 F1=0.9438），说明预训练知识在小样本场景中价值更大
